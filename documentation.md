...
**Activity Timeline — field-edit entries:** Every successful field save is logged as a permanent entry in the Activity Timeline (right column). Field-edit entries are visually distinct: they show a blue PencilLine icon instead of the coloured dot used by stage-change entries. Each entry shows the field label, who made the change, and a "Was → Now" block in monospace below the label line. Phone number changes are logged with the last 4 digits only (e.g. `***4344`) for privacy. No entry is created if the new value matches the old value.

**Stage History — inline expand/collapse:** Below the Activity Timeline sits the **Stage History** section, which lists every pipeline stage change for that lead. Each entry shows the staff member's avatar (initials, colour-coded by name), their name, the previous stage, an arrow, and the new stage — both rendered as colour-coded pills matching the pipeline colour scheme. Timestamps display as relative time ("13h ago", "2d ago"). By default only the **10 most recent** entries are shown. If more exist a **"Show full history (N older)"** button appears below the list; clicking it expands the list inline — older entries appear beneath an "older entries" divider — and the button toggles to **"Show less"**. No modal or drawer is used.
...
Sent messages render @mention chips as styled blue pills; if the mention is directed at the current viewer it renders in amber. Staff who no longer exist in the system render as struck-through grey. Group mention chips render in amber.
