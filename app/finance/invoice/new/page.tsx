...
import { ChevronLeft, Send, Save, Plus, X, Building2, ChevronDown } from 'lucide-react';
...
function LineItemRow({
  ...
  return (
    <div className="border border-slate-200 rounded-xl p-5 mb-3 relative bg-white">
      <button
        type="button"
        onClick={onRemove}
...
function PDFPreview({
  ...
  const roundingDiff = totals.totalDue - totals.rawTotal;

  return (
    <div className="bg-white shadow-lg rounded-lg overflow-hidden border border-slate-200 w-full">
      {/* Header band */}
      <div className="bg-[#1E3A8A] p-6 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="w-20 h-20 bg-white rounded-lg flex items-center justify-center font-bold text-[#1E3A8A] text-2xl flex-shrink-0">
            IMI
          </div>
          <div>
            <p className="text-white font-bold text-xl leading-tight">Improve ME Institute</p>
            <p className="text-white text-sm opacity-80 mt-1">Gold &amp; Diamond Park, Dubai</p>
          </div>
        </div>
        <div className="text-right text-white">
          <p className="font-bold text-2xl">TAX INVOICE</p>
          <p className="text-base opacity-90 mt-1">Invoice #IMI-0142</p>
          <p className="text-sm opacity-75 mt-1">Date: {fmtDate(issueDate)}</p>
          <p className="text-sm opacity-75">Due: {fmtDate(dueDate)}</p>
        </div>
      </div>

      {/* Bill To / TRN */}
      <div className="grid grid-cols-2 gap-6 p-5 bg-slate-50 border-b border-slate-200">
        <div>
          <p className="text-xs font-bold text-slate-400 uppercase mb-2">Bill To</p>
          <p className="font-semibold text-slate-900 text-base">{student?.guardian ?? '—'}</p>
          <p className="text-sm text-slate-600 mt-0.5">{student?.name ?? '—'}</p>
        ...
        <div className="text-right">
          ...
          <p className="text-xs text-slate-400 mt-2">Payment Ref:</p>
          <p className="font-semibold text-slate-900">IMI-0142</p>
        </div>
      </div>
      ...
      <div className="p-5">
        <table className="w-full">
          <thead>
            <tr className="bg-[#1E3A8A] text-white text-xs">
              {['Subject', 'Year', 'Sessions', 'Rate/Session', 'Amount'].map((h, i) => (
                <th
                  key={h}
                  className={cn('font-medium py-2.5 px-3 text-left', i >= 2 && 'text-right')}
                >
                  {h}
                </th>
              ))}
          ...
          <tbody>
            {lineItems.length === 0 ? (
              <tr>
                <td colSpan={5} className="text-center py-8 text-slate-300 text-sm italic">
              ...
              lineItems.map((item, i) => (
                <tr
                  key={item.id}
                  className={cn('border-b border-slate-100 text-sm', i % 2 === 0 ? 'bg-white' : 'bg-slate-50/50')}
                >
                  <td className="py-3 px-3">{item.subject || '—'}</td>
                  <td className="py-3 px-3">{item.yearGroup}</td>
                  <td className="py-3 px-3 text-right">{item.sessions}</td>
                  <td className="py-3 px-3 text-right">AED {item.rate.toLocaleString('en-AE')}</td>
                  <td className="py-3 px-3 text-right font-medium">{fmtAED(item.sessions * item.rate)}</td>
                </tr>
              ))
            )}
            {enrolmentFee && (
              <tr className="border-b border-slate-100 bg-white text-sm">
                <td className="py-3 px-3 italic text-slate-500">Enrolment Fee</td>
                <td className="py-3 px-3 text-slate-400">—</td>
                <td className="py-3 px-3 text-right text-slate-400">—</td>
                <td className="py-3 px-3 text-right text-slate-400">—</td>
                <td className="py-3 px-3 text-right font-medium">{fmtAED(300)}</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      <div className="p-5 bg-white border-t-2 border-slate-200">
        <div className="flex justify-between gap-6">
          ...
          <div className="text-sm text-slate-500 italic max-w-[200px] space-y-1">
            {notes && <p>{notes}</p>}
          ...
          <div className="w-64 space-y-1.5">
            ...
            {Math.abs(roundingDiff) >= 0.01 && (
              <div className="flex justify-between text-sm text-slate-500">
                ...
                <span>{roundingDiff >= 0 ? '+' : '−'} AED {Math.abs(roundingDiff).toFixed(2)}</span>
              </div>
            )}
            <div className="border-t-2 border-slate-800 my-2" />
            <div className="flex justify-between text-base font-bold">
              <span className="text-slate-900">TOTAL DUE</span>
              <span className="text-[#1E3A8A]">{fmtAED(totals.totalDue)}</span>
            </div>
          </div>
        </div>
        ...
        {instalments.length > 0 && (
          <div className="mt-4 pt-4 border-t border-slate-200">
            <p className="text-xs uppercase text-slate-400 tracking-wide mb-2">Payment Schedule</p>
            {instalments.map(inst => (
              <div key={inst.label} className="flex justify-between text-sm py-1">
                <span className="text-slate-700">{inst.label}</span>
                <span className="text-slate-400">{fmtDate(inst.due)}</span>
                <span className="font-medium text-slate-900">{fmtAED(totals.totalDue * inst.pct / 100)}</span>
              </div>
            ))}
          </div>
        )}
      ...
      {bankInfo && (
        <div className="p-5 bg-slate-50 border-t border-slate-200">
          <p className="text-xs font-bold text-slate-500 uppercase mb-3">Please Transfer Payment To:</p>
          <div className="grid grid-cols-2 gap-y-2 text-sm">
            {(
              ...
              <div key={label} className="contents">
                ...
                <span className={cn('text-slate-700', label !== 'Bank' && 'font-mono text-xs')}>{value}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Footer */}
      <div className="p-4 bg-[#1E3A8A] text-white text-xs text-center space-y-1">
        <p className="opacity-90">Improve ME Institute LLC · Gold &amp; Diamond Park · Dubai, UAE</p>
        <p className="opacity-70">Thank you for choosing Improve ME Institute</p>
      </div>
    </div>
  );
}
...
export default function NewInvoicePage() {
  ...
  const totals = useMemo<Totals>(() => {
    ...
    return { subtotal, discountValue, totalExclVAT, vat, rawTotal, totalDue };
  ...
  return (
    <div className="flex flex-col" style={{ height: '100vh', overflow: 'hidden' }}>
      {/* Custom top bar */}
      <div className="h-14 bg-white border-b border-slate-200 px-6 flex items-center justify-between sticky top-0 z-30 flex-shrink-0">
        <div className="flex items-center">
          ...
          <span className="font-semibold text-slate-900 text-lg ml-2">New Invoice</span>
          <span className="text-slate-400 text-sm ml-3">IMI-0142</span>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            type="button"
            className="flex items-center gap-2 px-4 py-1.5 border border-slate-200 text-slate-600 text-sm rounded-lg hover:bg-slate-50 transition-colors cursor-pointer"
          >
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-8 bg-white space-y-8">
          ...
          {/* BILL TO */}
          <section>
            <p className="text-xs uppercase text-slate-400 tracking-wide mb-4">Bill To</p>
          ...
          {/* LINE ITEMS */}
          <section>
            <p className="text-xs uppercase text-slate-400 tracking-wide mb-4">Line Items</p>
            {lineItems.map(item => (
              <LineItemRow
                key={item.id}
                item={item}
          ...
          {/* ADJUSTMENTS */}
          <section>
            <p className="text-xs uppercase text-slate-400 tracking-wide mb-4">Adjustments</p>
          ...
          {/* PAYMENT PLAN */}
          <section>
            <p className="text-xs uppercase text-slate-400 tracking-wide mb-4">Payment Plan</p>
          ...
          {/* NOTES */}
          <section>
            <p className="text-xs uppercase text-slate-400 tracking-wide mb-4">Notes</p>
            <textarea
              rows={2}
        ...
        <div className="w-[500px] flex-shrink-0 bg-slate-50 border-l border-slate-200 overflow-y-auto p-8">
