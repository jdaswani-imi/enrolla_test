...
// Creates a reaction notification for the message author (not the reactor).
export async function POST(request: NextRequest) {
  const auth = await requireAuth();
  if (!auth.ok) return auth.response;

  const body = await request.json().catch(() => ({}));
  const { authorName, entityId, entityType, messageId, emoji, reactorName, messagePreview } =
    body as {
      authorName?: string;
      entityId?: string;
      entityType?: string;
  ...
  // Look up the message author by display name (first_name + last_name)
  ...
  // Don't notify if the reactor is the message author
  ...
  const surfaceLabel =
    surface === "lead" ? `lead chat` : "task chat";
  ...
  const { error } = await supabase.from("notifications").insert({
    ...
    metadata: {
      ...
      body: `"${(messagePreview ?? "").slice(0, 50)}" · ${surfaceLabel}`,
      deepLink,
    },
    unread: true,
  });
  ...
  if (error) {
    console.error("reaction notification insert error:", error.message);
