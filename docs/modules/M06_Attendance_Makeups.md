# ENROLLA
# M06 — Attendance, Makeups & Concern Engine
v2.1 | Confidential
...
M06 tracks student presence at every session, manages makeup eligibility, enforces absence policies, and feeds attendance data into the Concern Engine (M06.A), the student profile (M17), and the Management Dashboard (M10). Attendance data is the foundation of the platform's welfare and financial accuracy — session deductions occur at attendance confirmation, not at scheduling.
...
| Version | v2.2 |
| Status | Current |
| AMDs absorbed | AMD-03.06 (48hr window — superseded: hard lock removed, replaced with tiered reminders), AMD-05.26 (unlimited package welfare fallback), AMD-05.27 (absence alert maths fix) |
...
| Feeds into | M06.A Concern Engine, M10 Dashboard, M17 Student Profile, M08 Finance |
| Dependencies | M04, M05, M08, M17, M20 |
| Phase | v1 |
...
Attendance is marked per session by the teacher or Admin from the session register in M05.
...
| No hard lock | Attendance can be marked or corrected at any time by any role with `attendance.mark` or `attendance.correct`. There is no time-based lock on attendance records. |
| Tiered reminders | Unmarked sessions surface colour-coded in-app reminder banners scoped to the assigned teacher, the department HOD, and Admin: yellow banner at 24 hours past session end; amber banner at 48 hours; red banner at 72 hours and beyond. Banners remain visible until attendance is marked. |
| Auto-flag | After 24 hours without marking, the session is flagged in the Admin dashboard unmarked sessions view. The flag remains until attendance is confirmed. |
...
| Unbilled sessions | If a student has no active invoice for the subject being marked, a warning banner appears before confirmation. The session is still marked and logged in the Unbilled Sessions tracker (M04). |
...
| Retention | All absence records are part of the student's cumulative record (M02) and are never deleted. |
...
| Configurable range | 2 to 5 consecutive absences. Configured in M20 under Attendance Settings. Cannot be set to 0 or 1. |
...
| **Element** | **Detail** |
|---|---|
| Carry-over basis | Carry-over value: Fixed whole number only (0, 1, or 2). Percentage-based carry-over is not supported and not configurable. The carry-over value cannot exceed the number of unused makeups the student has at term end. Configurable in M20. |
...
| Expiry | Configurable in M20. IMI default: end of the following term. |
...
| No-show concern trigger | 2 no-shows in the same subject in the current term auto-prompts a concern to HOD (configurable in M20). See M06.A Section 2. |
...
| Session selection | Admin selects a suitable session from available slots in M05 for the same subject. The makeup session is linked to the original missed session on the student's record. |
...
| **Setting** | **IMI Value** |
|---|---|
| Attendance marking window | No hard lock — attendance can be marked or corrected at any time. Tiered reminders fire at 24h (yellow), 48h (amber), 72h+ (red) past session end. |
| Reminder scope | Assigned teacher, HOD for the department, and Admin all see the reminder banners for unmarked sessions in their scope. |
| Primary makeup allowance | 1 per term per subject |
| Secondary makeup allowance | 2 per term per subject |
| Unlimited package welfare fallback | 3 consecutive absences (configurable 2–5 in M20) |
| Smaller alert guard condition | Only fires if (allowance − 1) > 0. Primary: smaller alert never fires. |
| No-show logging window | Admin should log the no-show reason within 48 hours as best practice, but there is no hard lock. After 48 hours without a logged reason, the no-show is auto-confirmed and flagged on the Admin dashboard. |
...
All attendance actions are hidden for unauthorised roles — never greyed out. HR/Finance has no attendance access at all — the attendance nav item is hidden entirely for this role.

## Page Access

`attendance.view` — Super Admin, Admin Head, Admin, Academic Head, HOD, Teacher, TA. HR/Finance is excluded.

## Action Permissions

| **Action** | **Permission key** | **Permitted roles** |
|---|---|---|
| View attendance | `attendance.view` | Super Admin, Admin Head, Admin, Academic Head, HOD, Teacher, TA |
| Mark attendance | `attendance.mark` | Super Admin, Admin Head, Admin, Academic Head, HOD, Teacher |
| Correct attendance | `attendance.correct` | Super Admin, Admin Head, Admin, HOD |
| Unlock 48h window | `attendance.unlockWindow` | Super Admin, Admin Head |
| Book makeup | `attendance.bookMakeup` | Super Admin, Admin Head, Admin, Academic Head, Teacher, TA |
| Override makeup eligibility | `attendance.overrideMakeup` | Super Admin, Admin Head, HOD |

**Notes:**
- TA can view attendance and book makeups but cannot mark or correct attendance records.
- Academic Head can mark attendance but cannot correct it or unlock the window.
- HOD can correct attendance and override makeup eligibility but cannot unlock the 48h window.
- HR/Finance has no attendance permissions of any kind. The nav item is fully hidden.

---

# M06.A — Concern Engine

The Concern Engine is the welfare escalation system linked to M06. It manages formal concerns about student welfare, academic performance, attendance, and behaviour through a three-level escalation structure. Concerns feed into M17 (Student Profile Concerns tab), M10 (dashboard welfare metrics), and M19.A (Academic Alert System).
...
| Auto-escalation timers | Configured independently for L1→L2 and L2→L3 in M20 by Super Admin or Admin Head. IMI to set at go-live. |
...
The following metrics are monitored per student per subject and can be configured to auto-prompt a concern in M20.
...
| Progress tracker below threshold | Student's progress tracker (M19) shows sustained below-pass performance | Below pass threshold for 3 consecutive tracked sessions |

Each metric trigger is configurable per tenant in M20 — the threshold, whether it auto-raises a concern or only prompts the teacher/HOD, and whether it fires per subject or across all subjects. When a metric threshold is reached, the system prompts the HOD with an in-app notification. HOD decides whether to log a formal concern or dismiss the prompt with a reason.
...
| Progress tracker trigger | Below pass threshold for 3 consecutive tracked sessions (M19) |
